...
exports.changeProfilePicture = async (req, res) => {
  const thumbnailPath = req.file.path.replace('profile-picture/','profile-picture/100x100/')
  var thumbnail = await cloudinary.uploader.upload(req.file.path, { public_id: thumbnailPath.replace('.png','').replace('.jpeg','').replace('.jpg','').replace('.gif',''),width:100,height:100 })
