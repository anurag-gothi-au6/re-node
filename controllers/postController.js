    ...
    query = [
      {
        $facet: {
          posts: [
            {
              $match: {
                author: { $in: [...req.body.followings, req.body.user] },
              },
            },
            { $sort: { createdAt: -1 } },
            { $limit: 10 },
...
exports.createPost = async (req, res) => {
  try{
  const thumbnailPath = req.file.path.replace('post-images/','post-images/thumbnail/')
  var thumbnail = await cloudinary.uploader.upload(req.file.path, { public_id: thumbnailPath.replace('.png','').replace('.jpeg','').replace('.jpg','').replace('.gif',''),width:480,height:480 })
  var Uploaded = await cloudinary.uploader.upload(req.file.path, { public_id: req.file.path.replace('.png','').replace('.jpeg','').replace('.jpg','').replace('.gif','')});
